...
word_remove_filter = ['Materiell:', 'Ruteterminperiode:', 'start:', 'Rutetermin:',
...
def sorter_side(page):
            ...
            for uke, uke_verdi in uker.items():
                for dager in dag_pos:
                    for dag, dag_verdi in dager.items():
                        if objektet_innenfor_uke_dag(text_obj, uke_verdi, dag_verdi, word_remove_filter, word_allow_filter):
                            if search_obj == 'tid':
                                plasseringslogikk_tid(text_obj, uke_verdi, dag_verdi, turnus)
                            elif search_obj == 'dagsverk':
                                plasseringslogikk_dagsverk(text_obj, uke, dag, turnus)

    
    def objektet_innenfor_uke_dag(word, uk):
        return (
            word['top'] >= uke_verdi[0]
            and word['bottom '] <= uke_verdi[1]
            and word['x0'] >= dag_verdi[0]
            and word['x0'] <= dag_verdi[1]
            and word['text'] not in word_remove_filter
        )
    ...
    def plasseringslogikk_tid(word, uke, dag, turnus):
        ...
        if any(sub in word["text"] for sub in word_allow_filter):
            ...
            # Hvis det ikke er dag1
                ...
                if any(val in turnus[uke][dag-1]['tid'] for val in word_allow_filter):
                    turnus[uke][dag]['tid'].append(word['text'])
                    turnus[uke][dag]['x0'].append(word['x0'])
    ...
    # Algoritme for sortering av dagsverk
    ...
    # Legger turnusene fra denne siden i en liste med dicts med alle turnusene
    try:
        turnuser.append({turnus_1_navn:turnus1})
        turnuser.append({turnus_2_navn:turnus2})
    except UnboundLocalError:
        # Hvis det kun er en turnus på siden siden.
        pass
